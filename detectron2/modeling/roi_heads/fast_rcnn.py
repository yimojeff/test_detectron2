...
FAST_LAYERS_REGISTRY = Registry("FAST_LAYERS")
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, box_features):
        ...
        if self.tsne_vis:
            storage = get_event_storage()
            storage._vis_tsne_feature["feature"].append(box_features.clone().detach())
            storage._vis_tsne_feature["label"].append(gt_classes.clone().detach())
...
@FAST_LAYERS_REGISTRY.register()
class TESTFastRCNNOutputLayers(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
    ...
    def __init__(
        ...
        *,
        ...
        ##
        #self.num_classes = num_classes
        feat_dim = [32, int(input_shape.channels/32)]
        decom = True; use_gpu= True; self.num_kernel=32

        self.feat_dim = feat_dim

        assert len(feat_dim) == 2
        self.centers_shape = feat_dim.copy()
        self.centers_shape.insert(0, num_classes+1)
        # self.covars_shape = self.centers_shape.copy()
        # self.covars_shape.append(feat_dim[1])
        self.weight_layer = nn.Linear(input_shape.channels, self.num_kernel, bias=False)
        ...
        if use_gpu:
            ...
            self.gmm_weights = nn.Parameter(torch.softmax(torch.randn(num_classes+1, feat_dim[0]), dim=-1).cuda(), requires_grad=False)
        else:
            self.centers = nn.Parameter(torch.randn(self.centers_shape))
            self.gmm_weights = nn.Parameter(torch.softmax(torch.randn(num_classes+1, feat_dim[0]), dim=-1), requires_grad=False)
        self.tsne_vis = tsne_vis
    ...
    def forward(self, x):
        ...
        scores = self.get_L2_score(self.centers,  # covs_inv,
                                  self.gmm_weights.clone(),
                                  x.reshape(-1, self.feat_dim[0], self.feat_dim[1]))
    ...
    def losses(self, predictions, proposals, box_features):
        ...
        center = self.centers[gt_classes]  # centers here transfer to 4096,32,256

        gmm_weights = self.weight_layer(box_features)
        #logger.info(gt_classes.item()[0],gt_classes.item()[1])

        for cls in torch.unique(gt_classes):
            self.gmm_weights[cls] = F.softmax(gmm_weights[gt_classes == cls], dim=-1).mean(0).clone().detach() * 1e-4 + \
                                    self.gmm_weights[cls].clone().detach() * (1 - 1e-4)#?
        ...
        loss_js = torch.clamp(js, min=1e-5, max=1e+5).mean()
        total_loss = loss_gmm + loss_js
...
def build_fast_layers(cfg, input_shape):
    """
    Build a mask head defined by `cfg.MODEL.ROI_MASK_HEAD.NAME`.
    """
    name = cfg.MODEL.FAST_LAYERS.NAME
    return FAST_LAYERS_REGISTRY.get(name)(cfg, input_shape)